...
const MATERIAL_TEXT = {
  ...
  id407: {
    en: {
      ...
      shortName: 'ID 407',
      expectedPages: '4 pages',
      note: 'Online renewal applications still require the original contract for inspection.'
    }
  },
  helperTravelOriginal: {
    en: {
      ...
      templateId: 'Non-fixed template',
      expectedPages: 'As per document'
    }
  },
  helperTravelCopy: {
    en: {
  ...
  financialProof: {
    en: {
      ...
      templateId: 'Tax demand note / bank statement / salary proof',
  ...
  addressProof: {
    en: {
      ...
      templateId: 'Rates demand note / utility bill, etc.',
      expectedPages: 'As evidenced'
    }
  },
  testimonial: {
    en: {
      name: "Helper's testimonial",
...
function localizedMaterial(material) {
  if (currentLanguage.value === 'zh') return toTraditionalObject(material)
  const localized = {
    ...material,
    ...(MATERIAL_TEXT[material?.id]?.en || {})
  }
  return {
    ...localized,
    statusText: localizedMaterialStatusText(localized),
    scopeText: localizedMaterialScopeText(localized),
    issue: localizedTextValue(localized.issue)
  }
}
...
function localizedMaterialStatusText(material) {
  if (MATERIAL_TEXT[material?.id]?.en?.statusText) return MATERIAL_TEXT[material.id].en.statusText
  if (material.status === 'pass') return 'Uploaded and recognised'
  if (material.status === 'fail') return material.core ? 'Missing core document' : 'Missing document'
  if (material.status === 'review') return 'Template recognition confidence is low'
  if (material.status === 'warn') {
    return material.conditional ? 'Conditionally required; not uploaded' : 'Not uploaded; non-blocking'
  }
  if (material.status === 'muted') return 'Not applicable'
  return localizedTextValue(material.statusText)
}

function localizedMaterialScopeText(material) {
  if (MATERIAL_TEXT[material?.id]?.en?.scopeText) return MATERIAL_TEXT[material.id].en.scopeText
  if (material.core) return t('affectsFinalDecision')
  if (material.applicable) return t('officialChecklistNonBlocking')
  return t('notApplicableCurrentType')
}
